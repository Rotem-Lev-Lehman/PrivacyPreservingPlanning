...
namespace Planning
{
    public class Problem
    {
        ...
        {
            Problem p = new Problem(Name, Domain);
            p.Goal = Goal.Clone();
            p.m_lKnown = new HashSet<Predicate>(m_lKnown);
            p.m_lHidden = new List<CompoundFormula>(m_lHidden);
            p.ReasoningActions = new List<Action>(ReasoningActions);
            p.MetricStatement = MetricStatement;
            return p;
        }
        ...
        public void CleanUnUsePredicate(Domain d)
        {
            ...
            m_lKnown = startState;
                ...
                private void AddReasoningAction(List<Predicate> lAssignment, List<Predicate> lKnown, List<Predicate> lEffects, Dictionary<List<Predicate>, List<Predicate>> dActions)
                {
        ...
        */
        ...
        public void WriteSimpleProblem(string sProblemFile, State sCurrent)
        {
            StreamWriter sw = new StreamWriter(sProblemFile);
            sw.WriteLine("(define (problem K" + Name + ")");
            sw.WriteLine("(:domain K" + Domain.Name + ")");
            ...
            foreach (GroundedPredicate gp in sCurrent.Predicates)
            {
                if (!gp.Negation)
                {
                    ...
                    foreach (Predicate p in Domain.Predicates)
                        if (p.Name == gp.Name)
                            bNonPrivatePredicate = true;
                    if(bNonPrivatePredicate)
                        sw.WriteLine("\t" + gp.ToString());
                }
            }
            sw.WriteLine(")");
