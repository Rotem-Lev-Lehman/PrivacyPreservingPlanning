...
namespace Planning
{
    public class Action
    {
        ...
        public Dictionary<string, HashSet<GroundedPredicate>> mapEff = null;
       // public int ID { get; private set; }
        public string agent=null;
        public bool isDeletePublic = false;
        public bool isGoalAction = false;
        ...
        public List<Predicate> HashPrecondition = null;
        public List<Predicate> HashEffects = null;
        public List<Predicate> HashObserve = null;
        public int id;
        public int id2;
        ...
        public ParametrizedAction GroundedFrom { get; set; }
        public Dictionary<string, Constant> Bindings { get; set; }
        ...
        public List<Predicate> GetApplicableEffects(IEnumerable<Predicate> lPredicates, bool bContainsNegations, Dictionary<Predicate,Formula> dEffectPreconditions)
        {
            List<Predicate> lEffects = new List<Predicate>();
            List<CompoundFormula> lConditions = new List<CompoundFormula>();
            List<Formula> lObligatory = new List<Formula>();
        ...
        public Action KnowWhetherTagObservationTranslation(Dictionary<string, List<Predicate>> dTags, Domain d, string sActionTag)
        {
            ...
            foreach (string sTag in dTags.Keys)
            {
                ...
                {
                    CompoundFormula cfCondition = new CompoundFormula("when");
                    ParametrizedPredicate pNotTag = new ParametrizedPredicate("KNot");
                    pNotTag.AddParameter(new Constant(Domain.TAG, sTag));
                    pNotTag.AddParameter(new Constant(Domain.TAG, sActionTag));
            ...
            */
            for (int i = 0; i < lIncludedTags.Count - 1; i++)
            {
                for (int j = i + 1; j < lIncludedTags.Count; j++)
                {
                    ...
                    GroundedPredicate ppKnowNot1Given2 = new GroundedPredicate("KNot");
                    GroundedPredicate ppKnowNot2Given1 = new GroundedPredicate("KNot");
                    Constant pTag1 = new Constant(Domain.TAG, sTag1);
                    Constant pTag2 = new Constant(Domain.TAG, sTag2);
                    ppKnowNot1Given2.AddConstant(pTag1);
                    ppKnowNot1Given2.AddConstant(pTag2);
                    ppKnowNot2Given1.AddConstant(pTag2);
                    ppKnowNot2Given1.AddConstant(pTag1);
                    cfEffect.AddOperand(ppKnowNot1Given2);
                    cfEffect.AddOperand(ppKnowNot2Given1);
        ...
        */
        ...
        public CompoundFormula GetKnowWhetherPreconditions(Dictionary<string, List<Predicate>> dTags, Domain d, List<string> lIncludedTags, List<string> lExcludedTags)
        {
            ...
            if (Preconditions != null)
            {
                ...
                foreach (string sTag in lIncludedTags)
                {
                    ...
                    if (sTag != lIncludedTags[0])
                    {
                        ...
                        pKNotT.AddParameter(new Constant(Domain.TAG, sTag));
                        pKNotT.AddParameter(new Constant(Domain.TAG, lIncludedTags[0]));
            ...
            foreach (string sTag in lExcludedTags)
            {
                ...
                ParametrizedPredicate pNotTag = new ParametrizedPredicate("KNot");
                pNotTag.AddParameter(new Constant(Domain.TAG, sTag));
                pNotTag.AddParameter(new Constant(Domain.TAG, lIncludedTags[0]));
        ...
        public CompoundFormula GetKnowWhetherPreconditions(Dictionary<string, List<Predicate>> dTags, Domain d, string sActionTag)
        {
            ...
            cfKWPreconditions.SimpleAddOperand(cfOr);
             * */
            if (cfKWPreconditions.Operands.Count > 0)
                return cfKWPreconditions;
            return null;
        ...
        private CompoundFormula CreateTaggedKnowledgeWhetherGainConditions(CompoundFormula cfCondition, Domain d, IEnumerable<string> lTags, string sActionTag)
        {
            ...
            foreach (string sKWTag in lTags)
            {
                ...
                {
                    ParametrizedPredicate pNotKWTag = new ParametrizedPredicate("KNot");
                    pNotKWTag.AddParameter(new Constant(Domain.TAG, sKWTag));
                    pNotKWTag.AddParameter(new Constant(Domain.TAG, sActionTag));
        ...
        private CompoundFormula CreateTaggedKnowledgeWhetherGainConditions(CompoundFormula cfCondition, Domain d, IEnumerable<string> lTags, string sActionTag)
        {
            List<Predicate> lPreconditions = new List<Predicate>();
            List<Predicate> lEffects = new List<Predicate>();
            cfCondition.Operands[0].GetAllPredicates(lPreconditions);
            cfCondition.Operands[1].GetAllPredicates(lEffects);
            CompoundFormula cfAllConditions = new CompoundFormula("and");
            ...
            foreach (string sKWTag in lTags)
            {
                ...
                Parameter pTag = new Parameter(Domain.TAG, Domain.TAG_PARAMETER);
                foreach (string sTag in lTags)
                {
                    CompoundFormula cfAnd = new CompoundFormula("and");
                    foreach (Predicate p in lPreconditions)
        ...
        private CompoundFormula CreateTaggedKnowledgeWhetherLossCondition(CompoundFormula cfCondition, Domain d, IEnumerable<string> lTags, string sActionTag)
        {
            ...
            foreach (string sForgetTag in lTags)
            {
                ...
                if (bContainsOption)
                {
                    ...
                    {
                        pNotTag = new ParametrizedPredicate("KNot");
                        pNotTag.AddParameter(new Constant(Domain.TAG, sForgetTag));
                        pNotTag.AddParameter(new Constant(Domain.TAG, sActionTag));
        ...
        */
        ...
        public override int GetHashCode()
        {
            ...
            else
            {
                // string s = "(action " + Name + "\n" +
                // " precondition " + Preconditions;
                HaseCode = ToString2().GetHashCode();
                return HaseCode;
            }
        }
        public void LoadPrecondition()
        {
            if (this.Preconditions != null)
            {
                this.HashPrecondition = this.Preconditions.GetAllPredicates().ToList();
            }
            else
            {
                this.HashPrecondition = new List<Predicate>();
            }
            ...
            if (this.Effects != null)
            {
                ...
                HashEffects = this.Effects.GetAllPredicates().ToList();
            ...
            {
                HashObserve = this.Observe.GetAllPredicates().ToList();


            }
            else
            {
                HashObserve = new List<Predicate>();
            }
            HaseCode = ToString2().GetHashCode();

        }
        private bool Comparer(List<Predicate> a, List<Predicate> b)
        {
            List<Predicate> l1 = a.Except(b).ToList();
            List<Predicate> l2 = b.Except(a).ToList();
        ...
        public Action RemoveUniversalQuantifiers(HashSet<Constant> lConstants, List<Predicate> lConstantPredicates, Domain d)
        {
            Action aNew = Clone();
       ...
       */
         ...
         *         public Formula RegressDet(Action a)
        {
            ...
            foreach (CompoundFormula cfCondition in a.GetConditions())
            {
                ...
                {
                    cfOr.AddOperand(cfCondition.Operands[0].CreateRegression(pNegate, -1).Negate());
                }
                iCondition++;
            }
