            ...
            else
            {
                ...
                if (Operator != "oneof" && Operands.Contains(f.Negate()))
                {
                    if (Operator == "and")
                    {
                        ...
                        if (!((PredicateFormula)f).Predicate.Negation)
                        {
                            Operands.Add(f);
                        }
                        else
                        {
                            Operands.Add(f.Negate());
                        }
...
*/
      ...
      /*  public CompoundFormula ToCNF()
        ...
        */
        ...
        public override Formula FilterPredicates(HashSet<Predicate> hsToRemove)
        {
            CompoundFormula cfFiltered = new CompoundFormula(Operator);
            foreach (Formula fSub in Operands)
            {
                Formula fFiltered = fSub.FilterPredicates(hsToRemove);
                if (fFiltered != null)
                    cfFiltered.AddOperand(fSub);
            }
            if (Operands.Count == 0)
                return null;
            return cfFiltered;
