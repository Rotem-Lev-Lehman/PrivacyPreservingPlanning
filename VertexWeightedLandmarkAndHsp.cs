...
namespace Planning
{
    class VertexWeightedLandmarkAndHsp
    {
        public List<string> History = new List<string>();
        public HashSet<CompoundFormula> canGetInParallel;
        public static double maxLandmarks=0;
        public static double maxHsp = 0;
       ...
       // private List<Landmark> restLandmark = null;
        ...
        public VertexWeightedLandmarkAndHsp Parent { get; private set; }
        ...
        public VertexWeightedLandmarkAndHsp(int[] m_statesNubmber, List<Action> m_lplan, double new_G, int landmark_h, double hsp_h, double m_cost, List<HashSet<CompoundFormula>> pot)
        {
          /*  if (pot != null)
        ...
        public VertexWeightedLandmarkAndHsp Apply(Action act)
        {
           ...
           // if (m_h == 1)
            ...
            VertexWeightedLandmarkAndHsp newVertexHsp = new VertexWeightedLandmarkAndHsp(newStateNumbers, lplan, g + 1, (int)m_h, 0, 0, publicPredicateImplications);
            int hspH = newVertexHsp.ComputeHsp();
            if (cost == -1)
                return null;
            //if (cost == 0)
              //  Console.WriteLine("ss");
            newVertexHsp.h = (((double)hspH) / maxHsp) * 0.7 + (m_h/maxLandmarks)*0.3;
       ...
       /* public VertexWeightedLandmarkAndHsp LocalApply(Action act)
        {
            bool innovative = false;
            if (!agents[map[act.agent]].CanDo(statesNubmber[map[act.agent]], act))
                return null;
            ...
            VertexWeightedLandmarkAndHsp newVertexHsp = new VertexWeightedLandmarkAndHsp(newStateNumbers, lplan, g + 1, m_h, cost, publicPredicateImplications);
            newVertexHsp.Parent = this;
            if (newVertexHsp.h == -1)
                return null;
            newVertexHsp.lplan.Add(act);
        ...
        public static int LandmarksComparer(VertexWeightedLandmarkAndHsp v1,VertexWeightedLandmarkAndHsp v2)
        {
            if (v1.h2 == v2.h2)
            {
                if (v1.h > v2.h)
        ...
        public static int HspComparer(VertexWeightedLandmarkAndHsp v1, VertexWeightedLandmarkAndHsp v2)
        {
            if (v1.h == v2.h)
            {
                if (v1.h2 > v2.h2)
        ...
        public static int ComparerOp(VertexWeightedLandmarkAndHsp v1, VertexWeightedLandmarkAndHsp v2)
        {
            if (v1.h == v2.h)
            {
                if (v1.g == v2.g)
        ...
        public static int ComparerII(VertexWeightedLandmarkAndHsp v1, VertexWeightedLandmarkAndHsp v2)
        {
        ...
        {
            if (obj is VertexWeightedLandmarkAndHsp)
            {
                VertexWeightedLandmarkAndHsp v = (VertexWeightedLandmarkAndHsp)obj;
    ...
    class ComapareVertexWeightedLandmarkAndHsp : IComparer<VertexWeightedLandmarkAndHsp>
    {
        public static IComparer<VertexWeightedLandmarkAndHsp> GetVertexHspComparer()
        {
            return (IComparer<VertexWeightedLandmarkAndHsp>)new ComapareVertexHsp();
        }

        int IComparer<VertexWeightedLandmarkAndHsp>.Compare(VertexWeightedLandmarkAndHsp VertexHsp1, VertexWeightedLandmarkAndHsp VertexHsp2)
        {
            VertexWeightedLandmarkAndHsp v1 = (VertexWeightedLandmarkAndHsp)VertexHsp1;
            VertexWeightedLandmarkAndHsp v2 = (VertexWeightedLandmarkAndHsp)VertexHsp2;
